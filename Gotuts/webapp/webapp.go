...
func portuguesHandler(writer http.ResponseWriter, request *http.Request) {
...
func interactHandler(writer http.ResponseWriter, request *http.Request) {
	...
	errorCheck(err)
	todos := TodoList{TodoCount: len(todoVals), Todos: todoVals}
...
func main() {
	...
	http.HandleFunc("/hola", portuguesHandler)
